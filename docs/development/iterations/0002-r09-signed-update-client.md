...
`macos-release` 只允许 `main` branch promotion，secret 集合为空。
...
- [x] **R09-04 受保护 release policy**：tag-bound `macos-15` job、最小权限、
  `macos-signing` 唯一 Environment secret、secret-free `macos-release`、runtime/renderer/
  companion audit、自签名 DMG+ZIP 组装、独立验证。
- [x] **R09-05 两阶段发布完整性**：tag push 只上传一个已验证资产集合并创建/复核 Draft；
...
- [ ] **R09-06 物理门禁**：管理员 provision 生产 credential bundle，核验真实
  ...
  DMG/Gatekeeper 和物理 0.0.1 → 0.0.2 更新/fallback。
...
- release workflow 只从 exact `v*.*.*` tag 构建，普通 PR/fork/source job 无 secret；
  tag push 只能创建经远端完整性校验的 Draft，永远不能公开；
- build/sign 绑定 tag-only `macos-signing`，Draft job 无 Environment/secret；公开只能由
  branch-only、零 secret `macos-release` 的 `workflow_dispatch --ref main` 执行，
  `release_tag` 只是资料输入；
...
| --- | --- | --- | --- | --- |
| VAL-RELEASE-POLICY-001 Desktop | `pass` | 2026-07-31；当前工作树 | `cd desktop && ./node_modules/.bin/vitest run tests/releasePolicy.test.ts tests/updateTrustPackaging.test.ts` | 2 files / 18 pass；本地/远端资产、promotion order、Draft/Published、candidate digest 和 update trust source contract |
| VAL-RELEASE-POLICY-001 Backend | `pass` | 2026-07-31；当前工作树 | `backend/.venv/bin/pytest -q tests/backend/test_desktop_release_bootstrap.py tests/backend/test_desktop_release_workflow_policy.py` | 22 pass；source 合同：job/event/权限、两个 Environment、唯一 secret、ruleset/Immutable Releases 与两阶段远端复核；真实 settings `not-run` |
| VAL-RELEASE-PROMOTION-001 | `pass` | 2026-07-31；当前工作树 | 上述 Desktop/Backend focused commands；YAML parse；17 个 workflow `run` script `bash -n` | source 合同：tag push Draft-only、main-only promotion、trusted verifier/隔离 worktree/fresh peel、PATCH 前 fresh `origin/main` promotion order、固定 Release ID、published 预状态拒绝、post-publish attestation/immutable；真实 promotion `not-run` |
...
| Desktop full | `pass` | 2026-07-31；当前工作树 | `cd desktop && npm test -- --run` | 30 files / 245 pass / 7 skip |
...
| 发布资产在上传后漂移 | tag push 停止在 Draft；promotion 重新下载并绑定 candidate digest，以固定 Release ID 公开并在前后复核 |
