...
| TODO-PACKAGED-SMOKE-001 | W02 | `in-progress` | REQ-PACKAGED-SMOKE-001；ADR-0016 | isolated engineering-smoke config、strict manifest、staging/bundle audit、policy/workflow/tests；后续 launch/UDS/no-listener harness | boundary/assembly source `in-progress`；VAL-PACKAGED-SMOKE-001 `not-run` |
...
| `.github/workflows/packaged-smoke.yml`、`desktop/electron-builder.smoke.yml`、engineering-smoke staging/audit scripts、`desktop/src/{contracts,main/distribution,main/index,main/updateClient}.ts`、`web/src/desktopBridge.ts`、`runtime/engineering-smoke-manifest.schema.json`、packaging policy/tests 与 W02 governance/evidence | W02 engineering-smoke boundary/assembly，与 formal release 严格隔离；本 Work 不启动 packaged App | TODO-PACKAGED-SMOKE-001；ITER-0008/I01；W02-A implementation stage（非稳定 ID） | source implementation `in-progress`；`.app` assembly、packaged launch、VAL-PACKAGED-SMOKE-001 均 `not-run`；W10/W11 locked |
