...
R07–R11 是 ITER-0002 的追加记录，继承父迭代 `in-progress` 状态。
...
| REQ-DATA-001 | 标准 macOS 路径；desktop backup/restore；旧数据可盘点、原子迁移、回滚 | [ADR-0004](../adr/0004-runtime-paths-legacy-data-migration.md) | ITER-0003/D01–D03；TODO-DATA-*；TODO-LEGACY-CONTROL-001 | runtime path/migration/backup modules、legacy instance-control scripts | VAL-DATA-001、VAL-LEGACY-CONTROL-001 | Application Support/Caches/temp 部分实现；完整 gate `planned` / `not-run` |
| REQ-LEGACY-001 | Docker 暂留 legacy，迁移成功且另行决策后才弃用 | [ADR-0004](../adr/0004-runtime-paths-legacy-data-migration.md) | ITER-0006/L01–L05；TODO-LEGACY-CONTROL-001、TODO-LEGACY-EXIT-001 | existing Docker paths/docs、installer/runtime manifest/backup/restore controls | VAL-LEGACY-CONTROL-001、VAL-LEGACY-001 | `planned` / `not-run` |
...
| TODO-LEGACY-CONTROL-001 | legacy/P7 support | `planned` | REQ-LEGACY-001、REQ-DATA-001；ADR-0004 legacy/data 边界 | `scripts/lcf`、`scripts/{backup,restore}.sh`、`install.sh`、`install.command`、runtime manifest、legacy tests/docs | VAL-LEGACY-CONTROL-001 `not-run` |
...
| TODO-DATA-MIGRATION-001 | P4/P7 | `planned` | REQ-DATA-001、REQ-LEGACY-001；ADR-0004 | Backend/Desktop migration modules、journal/converters、legacy fixtures/recovery docs | VAL-DATA-001 migration subgate `not-run` |
...
| TODO-REL-DRAFT-001 | P5 | `blocked` | REQ-RELEASE-001、REQ-RELEASE-002、REQ-SECRET-001；ADR-0003/0009/0014 | `.github/workflows/desktop-release.yml`、tag-bound unique Draft、candidate manifest/evidence | VAL-RELEASE-POLICY-001 source `pass`；VAL-RELEASE-001 remote candidate subgate `not-run` |
...
| TODO-LEGACY-EXIT-001 | P7 | `blocked` | REQ-LEGACY-001、REQ-DATA-001、REQ-UPDATE-001；ADR-0004 + planned retention/deprecation ADR | migration/rollback rehearsal、exit review、deprecation docs/removal plan | VAL-LEGACY-001 `not-run` |
...
| VAL-LEGACY-CONTROL-001 | isolated legacy checkouts/Docker contexts；全部 Compose 插值变量、`COMPOSE_PROJECT_NAME`、custom bind/port/context/image/data/imports/runner/backup、错误实例与 manifest target mismatch matrix | `planned` / `not-run` |
