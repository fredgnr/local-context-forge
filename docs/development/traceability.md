...
| REQ-PACKAGED-SMOKE-001 | W01 technical PR/source、independent acceptance、accepted merge 与 resulting-main source 全部完成后、任何 destructive slice 前，提供最小且明确 non-release 的 packaged feedback loop | [ADR-0016](../adr/0016-pre1-incremental-retirement-engineering-package.md) | ITER-0008/I01；TODO-PACKAGED-SMOKE-001 | `.github/workflows/packaged-smoke.yml`、`desktop/electron-builder.smoke.yml`、engineering smoke prepare/beforePack/afterPack/audit scripts、engineering-only runtime disposition、strict manifest schema、policy checker/tests/evidence；formal release path unchanged | VAL-PACKAGED-SMOKE-001 | Draft PR #21 exact-head static assembly/bundle audit `pass`；packaged App launch/runtime `not-run`；W02 仍 `in-progress` |
...
| TODO-PACKAGED-SMOKE-001 | W02 | `in-progress` | REQ-PACKAGED-SMOKE-001；ADR-0016 | isolated engineering-smoke config、strict manifest、staging/bundle audit、policy/workflow/tests；后续 launch/UDS/no-listener harness | [static assembly/audit](evidence/W02/2026-08-06-08137c7-assembly.md) `pass`；VAL-PACKAGED-SMOKE-001 `not-run` |
...
| VAL-PACKAGED-SMOKE-001 | macOS arm64 engineering-smoke App；exact commit/digest/inventory、launch、renderer/preload、Main→private UDS health/domain request、quit/no orphan、无 public INET、exercised path 无系统 Python/Node/Git discovery、updater unavailable/no-network | `not-run`；[static assembly/audit record](evidence/W02/2026-08-06-08137c7-assembly.md) 与 pre-pack frozen sidecar staging smoke 不替代 packaged launch/runtime gate |
...
**W01 closed / W02 boundary-and-assembly in progress / release NO-GO**。
...
| `.github/workflows/packaged-smoke.yml`、`desktop/electron-builder.smoke.yml`、engineering-smoke staging/audit scripts、`desktop/src/{contracts,main/distribution,main/index,main/updateClient}.ts`、`web/src/desktopBridge.ts`、`runtime/engineering-smoke-manifest.schema.json`、packaging policy/tests 与 W02 governance/evidence | W02 engineering-smoke boundary/assembly，与 formal release 严格隔离；本 Work 不启动 assembled App | TODO-PACKAGED-SMOKE-001；ITER-0008/I01；W02-A implementation stage（非稳定 ID） | Draft PR #21 exact `08137c7…` / tree `d7814ac9…`：[source run `31024794734`](https://github.com/fredgnr/local-context-forge/actions/runs/31024794734) 与 [static assembly run `31024794972`](https://github.com/fredgnr/local-context-forge/actions/runs/31024794972) `pass`；inventory `7fcdb699…`；pre-pack sidecar staging smoke `pass`，但 packaged launch、VAL-PACKAGED-SMOKE-001 均 `not-run`；W10/W11 locked |
