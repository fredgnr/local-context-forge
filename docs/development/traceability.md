...
R07–R13 是 ITER-0002 的追加记录；R12/R13 已完成各自 source 纵切，父迭代仍为
`in-progress`。
...
| --- | --- | --- | --- | --- | --- | --- |
| REQ-GOV-001 | 每项变更有 active iteration、owner、稳定 task/VAL 和 commit-bound evidence | 现有治理规则；架构变化另由 ADR-0015/REQ-ELECTRON-ONLY 承载 | ITER-0001/T01；[R11](iterations/0002-r11-documentation-handoff.md)；[R12](iterations/0002-r12-legacy-retirement-scope.md)；[R13](iterations/0002-r13-pre1-incremental-retirement.md) W01 closure | `AGENTS.md`、`.agents/skills/**`、`docs/development/**`、PR template、W01 machine evidence | VAL-GOV-001、VAL-DOC-HANDOFF-001、VAL-LEGACY-SCOPE-001 | 历史 checkpoint 保留；W01 checkpoint `8573f608…` + PR #20 containing-commit check 为 `pass`；canonical activation 仍须独立验收、合入与 exact main CI |
| REQ-PRE1-SEQUENCING-001 | 固定 smoke → slices → cleaned engineering package → physical → final cutover/absence → production release 的顺序 | [ADR-0016](../adr/0016-pre1-incremental-retirement-engineering-package.md) | [R13](iterations/0002-r13-pre1-incremental-retirement.md)；[work plan](work-plan.md)；TODO-PRE1-SEQUENCING/GOV-EVIDENCE/CI-COVERAGE-001 | `docs/adr/0016-*`、`docs/development/{work-plan,todo,traceability,roadmap,status}.md`、plan checker | VAL-PRE1-SEQUENCE-001、VAL-GOV-001、VAL-CI-COVERAGE-001 | Accepted；三个 W01 PR gate 为 `pass`；W02 在 canonical activation 完成前仍不得启动 |
| REQ-PACKAGED-SMOKE-001 | W01 三个退出门禁通过后、任何 destructive slice 前，提供最小且明确 non-release 的 packaged feedback loop | [ADR-0016](../adr/0016-pre1-incremental-retirement-engineering-package.md) | ITER-0008/I01；TODO-PACKAGED-SMOKE-001 | future engineering-smoke config/harness/evidence；不修改 formal release path | VAL-PACKAGED-SMOKE-001 | `not-run`；W01 PR gate 已通过，但独立验收、canonical merge 与 exact main CI 尚未完成，且当前仓库无可用 harness |
...
| --- | --- | --- | --- | --- | --- |
| TODO-PRE1-SEQUENCING-001 | W01 | `done` | REQ-PRE1-SEQUENCING-001；ADR-0016 | ADR/work plan/R13/TODO/traceability/authority docs、`tools/check_pre1_work_plan.py`、negative unit fixtures、`ci-python` hook | VAL-PRE1-SEQUENCE-001 `pass`；checkpoint `8573f608…`、PR #20 run 30927840380 |
| TODO-GOV-EVIDENCE-001 | W01/P0 | `done` | REQ-GOV-001、REQ-PRE1-SEQUENCING-001；现有 evidence 治理规则 | `docs/development/{evidence/**,iterations/**,status.md,traceability.md}`、checkpoint + containing-commit evidence ledger | VAL-GOV-001 `pass`；commit/environment/command/count/skip/result 见 machine record |
| TODO-CI-COVERAGE-001 | W01/P0 | `done` | REQ-GOV-001、REQ-CI-001、REQ-PRE1-SEQUENCING-001；ADR-0009/0014 的 source/release CI 边界 | `Makefile`、`.github/{ci,workflows}/**`、QMD safe runner、guide-site exclusion、coverage checker/docs | VAL-CI-COVERAGE-001 `pass`；PR #20 exact-head source aggregate |
...
| --- | --- | --- |
| VAL-GOV-001 | exact source checkout；Python/MCP/Host Runner/demo SDK/Web/Desktop/QMD/governance commands、counts/skips/environment 与 evidence binding；Markdown links | `pass`；[W01 machine record](evidence/W01/2026-08-04.json)，checkpoint `8573f608…` + [PR #20 run 30927840380](https://github.com/fredgnr/local-context-forge/actions/runs/30927840380) |
...
| VAL-CI-COVERAGE-001 | exact source checkout + 最终 PR head CI；manifest 与 Make/workflow/QMD safety/guide-site external-blocked 反向一致；QMD 无 lifecycle build、test external network 或模型文件 | `pass`；checkpoint `8573f608…` 的 [run 30927840380](https://github.com/fredgnr/local-context-forge/actions/runs/30927840380)，QMD 10 pass / 1 allowlisted skip，0 model/cache |
| VAL-PRE1-SEQUENCE-001 | exact source checkout；`make pre1-work-plan-check` + links/version + `git diff --check 3eff97d… HEAD`；W01–W16 rank/mapping/state、formal not-run、W02 canonical activation | `pass`；checkpoint `8573f608…` 的 fixed base→head diff、42 governance tests 与 containing-commit check |
...
| `.github/{ci/source-coverage.json,workflows/desktop-ci.yml}`、`Makefile`、`tools/{check_ci_coverage,check_w01_evidence,render_source_coverage_evidence,run_qmd_source_ci}.py`、QMD network trap/package script、W01 docs/evidence | W01 source aggregate 机器化、QMD safe source tier、guide-site truthful exclusion、checkpoint→containing-commit exact-head evidence；不实现 W02/legacy deletion/package/control plane | TODO-PRE1-SEQUENCING/GOV-EVIDENCE/CI-COVERAGE-001；R13-06；PR #20 | 三个 W01 gate `pass`；checkpoint `8573f608…`、[run 30927840380](https://github.com/fredgnr/local-context-forge/actions/runs/30927840380) 与 [machine record](evidence/W01/2026-08-04.json)；W02 canonical activation 尚未满足 |
