...
| --- | --- | --- | --- | --- | --- |
| TODO-PRE1-SEQUENCING-001 | W01 | `in-progress` | REQ-PRE1-SEQUENCING-001；ADR-0016 | ADR/work plan/R13/TODO/traceability/authority docs、`tools/check_pre1_work_plan.py`、negative unit fixtures、`ci-python` hook | VAL-PRE1-SEQUENCE-001 final head `not-run` |
| TODO-GOV-EVIDENCE-001 | W01/P0 | `in-progress` | REQ-GOV-001、REQ-PRE1-SEQUENCING-001；现有 evidence 治理规则 | `docs/development/{evidence/**,iterations/**,status.md,traceability.md}`、checkpoint + containing-commit evidence ledger | VAL-GOV-001 final head `not-run`；每个 `pass` 必须绑定 commit/environment/command/result |
| TODO-CI-COVERAGE-001 | W01/P0 | `in-progress` | REQ-GOV-001、REQ-CI-001、REQ-PRE1-SEQUENCING-001；ADR-0009/0014 的 source/release CI 边界 | `Makefile`、`.github/{ci,workflows}/**`、QMD safe runner、guide-site exclusion、coverage checker/docs | VAL-CI-COVERAGE-001 `not-run` |
...
| --- | --- | --- |
| VAL-GOV-001 | exact source checkout；Python/MCP/Host Runner/demo SDK/Web/Desktop/QMD/governance commands、counts/skips/environment 与 evidence binding；Markdown links | `not-run`（W01 final head）；历史 checkpoint 不重标为 W01 evidence |
...
| VAL-CI-COVERAGE-001 | exact source checkout + 最终 PR head CI；manifest 与 Make/workflow/QMD safety/guide-site external-blocked 反向一致；QMD 无 lifecycle build、test external network 或模型文件 | `not-run`（W01 final head） |
| VAL-PRE1-SEQUENCE-001 | exact source checkout；`make pre1-work-plan-check` + links/version + `git diff --check 3eff97d… HEAD`；W01–W16 rank/mapping/state、formal not-run、W02 canonical activation | `not-run`（W01 final head） |
